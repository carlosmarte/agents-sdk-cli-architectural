/* Standalone copy of @llmorch/adapter-openai (Pattern 2). Kept identical to
...
/** Minimal structural view of the OpenAI client surface this adapter uses. */
...
/** OpenAI usage block → normalized TokenUsage. */
...
/**
 * Baseline adapter — the OpenAI Chat Completions shape is nearly 1:1 with the
 * normalized `ChatRequest`. Self-registers under id `openai` at module load.
 */
export class OpenAIAdapter implements LLMProvider {
  ...
  mapError(err: unknown): OrchestrationError {
    ...
    if (status === 401) return new AuthenticationError("openai: unauthorized", opts);
    if (status === 429) return new RateLimitError("openai: rate limited", opts);
    return new ProviderError("openai: request failed", opts);
  }
   ...
   * {@link ToolResult} as an OpenAI `tool` message, append to the serialized
   * history, and re-call to obtain the final answer.
   */
