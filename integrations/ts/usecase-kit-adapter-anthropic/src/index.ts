/**
 * @llmorch-int/usecase-kit-adapter-anthropic — TS use-case kit for Anthropic Claude.
 *
 * Pattern 1 of 3 (see ../README.md). Wraps `@llmorch/sdk`'s `createOrchestrator`
 * with the `anthropic` provider and exposes idiomatic, fully-typed helpers for the
...
/** Options for {@link createAnthropicKit}. */
export interface AnthropicKitOptions {
  ...
  /** Providers appended after `anthropic` to form an ordered fallback chain. */
...
/** The typed kit surface returned by {@link createAnthropicKit}. */
export interface AnthropicKit {
...
/**
 * Build a use-case kit for Anthropic Claude. With no options it reads the key from
 * `process.env.ANTHROPIC_API_KEY` and targets {@link DEFAULT_MODEL}; pass `fallback` to add
 * downstream providers, or `orchestrator` to inject your own.
 */
export function createAnthropicKit(
  options: AnthropicKitOptions = {},
): AnthropicKit {
