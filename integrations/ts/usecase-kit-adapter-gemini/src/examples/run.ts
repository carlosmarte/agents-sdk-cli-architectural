/**
 * Runnable demo for @llmorch-int/usecase-kit-adapter-gemini.
 *
 *   pnpm --filter @llmorch-int/usecase-kit-adapter-gemini build
 *   GEMINI_API_KEY=... node dist/examples/run.js
 *
 * Exits early with a hint if GEMINI_API_KEY is unset, so it is safe to invoke in CI.
 */
import { z } from "zod";

import { API_KEY_ENV, DEFAULT_MODEL, createGeminiKit } from "../index.js";
