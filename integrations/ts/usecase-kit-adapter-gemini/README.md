# @llmorch-int/usecase-kit-adapter-gemini

**Pattern 1 — TS use-case kit** for Google Gemini. Wraps `@llmorch/sdk` + the
`gemini` adapter and exposes idiomatic, typed helpers. No port logic is
duplicated — every call runs through the canonical orchestrator.

```ts
import { createGeminiKit } from "@llmorch-int/usecase-kit-adapter-gemini";

const kit = createGeminiKit(); // reads GEMINI_API_KEY, default model gemini-2.0-flash
...
```sh
pnpm --filter @llmorch-int/usecase-kit-adapter-gemini build
GEMINI_API_KEY=... node dist/examples/run.js
```
