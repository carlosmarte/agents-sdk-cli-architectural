/**
 * Runnable demo for @llmorch-int/usecase-kit-adapter-copilot.
 *
 *   pnpm --filter @llmorch-int/usecase-kit-adapter-copilot build
 *   GITHUB_TOKEN=... node dist/examples/run.js
 *
 * Exits early with a hint if GITHUB_TOKEN is unset, so it is safe to invoke in CI.
 */
import { z } from "zod";

import { API_KEY_ENV, DEFAULT_MODEL, createCopilotKit } from "../index.js";
