/**
 * Runnable demo for @llmorch-int/usecase-kit-adapter-openai.
 *
 *   pnpm --filter @llmorch-int/usecase-kit-adapter-openai build
 *   OPENAI_API_KEY=... node dist/examples/run.js
 *
 * Exits early with a hint if OPENAI_API_KEY is unset, so it is safe to invoke in CI.
 */
import { z } from "zod";

import { API_KEY_ENV, DEFAULT_MODEL, createOpenAIKit } from "../index.js";
