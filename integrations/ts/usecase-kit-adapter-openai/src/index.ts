/**
 * @llmorch-int/usecase-kit-adapter-openai — TS use-case kit for OpenAI.
 *
 * Pattern 1 of 3 (see ../README.md). Wraps `@llmorch/sdk`'s `createOrchestrator`
 * with the `openai` provider and exposes idiomatic, fully-typed helpers for the
...
/** Options for {@link createOpenAIKit}. */
export interface OpenAIKitOptions {
  ...
  /** Providers appended after `openai` to form an ordered fallback chain. */
...
/** The typed kit surface returned by {@link createOpenAIKit}. */
export interface OpenAIKit {
...
/**
 * Build a use-case kit for OpenAI. With no options it reads the key from
 * `process.env.OPENAI_API_KEY` and targets {@link DEFAULT_MODEL}; pass `fallback` to add
 * downstream providers, or `orchestrator` to inject your own.
 */
export function createOpenAIKit(
  options: OpenAIKitOptions = {},
): OpenAIKit {
