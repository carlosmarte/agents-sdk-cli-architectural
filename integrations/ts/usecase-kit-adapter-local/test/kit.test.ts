...
import { DEFAULT_MODEL, PROVIDER_ID, createLocalKit } from "../src/index.js";
...
describe("usecase-kit-adapter-local", () => {
  it("targets the local provider and default model", () => {
    const { orch } = fakeOrchestrator();
    const kit = createLocalKit({ orchestrator: orch });
