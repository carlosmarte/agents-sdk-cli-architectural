/**
 * Runnable demo for @llmorch-int/usecase-kit-adapter-local.
 *
 *   pnpm --filter @llmorch-int/usecase-kit-adapter-local build
 *   LLMORCH_LOCAL_API_KEY=... node dist/examples/run.js
 *
 * Exits early with a hint if LLMORCH_LOCAL_API_KEY is unset, so it is safe to invoke in CI.
 */
import { z } from "zod";

import { API_KEY_ENV, DEFAULT_MODEL, createLocalKit } from "../index.js";
